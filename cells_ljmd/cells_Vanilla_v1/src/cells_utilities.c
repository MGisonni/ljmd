...
void distribute_atoms(mdsys_t *sys, cells_t *cells) {
    ...
    // define an auxiliary array to store the cell index of each atom
    int *cell_index_array = (int *) malloc(sys->natoms*sizeof(int));

    // first compute the number of atoms in each cell
    for (i=0; i<sys->natoms; i++) {
        ...
        // update the counter of atoms in the cell
        cells[cell_index].catoms++;
        // store the cell index of the atom
        cell_index_array[i] = cell_index;
    }

    // allocate memory for the list of atoms in each cell
    for (i=0; i<ncell; i++) {
        cells[i].a_list = (int *) malloc(cells[i].catoms*sizeof(int));
        // reset counter for later redistribution
        cells[i].catoms = 0;
    }


    // finally, fill the list of atoms in each cell
    for (i=0; i<sys->natoms; i++) {
        // get cell index of the atom
        int cell_index = cell_index_array[i];
        // store atom index in the list of atoms in the cell
